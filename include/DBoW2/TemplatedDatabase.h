...
public:
   ...
   * @param filename
   */
  void saveWithName(const string &filename, const std::vector<std::string> &fileNames) const;
   ...
   * @param name node name
   */
  virtual void saveWithName(cv::FileStorage &fs,
...
template<class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::saveWithName(const string &filename, const std::vector<std::string> &fileNames) const
{
  cv::FileStorage fs(filename.c_str(), cv::FileStorage::WRITE);
  if(!fs.isOpened()) throw string("Could not open file ") + filename;

  saveWithName(fs, fileNames);
}
...
template<class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::saveWithName(cv::FileStorage &fs,
  const std::vector<std::string> &fileNames, const std::string &name) const
{
  // Format YAML:
  // vocabulary { ... see TemplatedVocabulary::save }
  // database
  ...
  for(dit = m_dfile.begin(); dit != m_dfile.end(); ++dit)
  {
    fs << "{" << "filename" << fileNames[imgIdx] << "data" << "["; // entry of DF
